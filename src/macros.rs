...
macro_rules! propagate_subcmd {
    ($app:expr, $sc:expr) => {{
        ...
        {
            let vsc = $app.settings.is_set(AppSettings::VersionlessSubcommands);
            let gv = $app.settings.is_set(AppSettings::GlobalVersion);

            if vsc {
                $sc.set(AppSettings::DisableVersion);
            }
            if gv && $sc.version.is_none() && $app.version.is_some() {
                $sc.set(AppSettings::GlobalVersion);
                $sc.version = Some($app.version.unwrap());
            }
            $sc.settings = $sc.settings | $app.g_settings;
            $sc.g_settings = $sc.g_settings | $app.g_settings;
            $sc.term_w = $app.term_w;
            $sc.max_w = $app.max_w;
        }
        {
            for a in $app.args.args.iter().filter(|a| a.global) {
