...
impl<'b> App<'b> {
    ...
    /// Replaces an argument to this app with other arguments.
    ///
    /// # Examples
    ///
    /// ```rust
    /// # use clap::{App, Arg, };
    /// let m = App::new("app")
    ///     .replacer("install", &["module", "install"])
    ...
    /// ```
    ///
    /// Here, when args given are `app install`, they will be changed to `app module install`.
    pub fn replacer(mut self, name: &'b str, target: &'b [&'b str]) -> Self {
